...
void screen_move_cursor(int dx, int dy) {
    ...
    int row = get_offset_row(off);
    int col = get_offset_col(off);
    row += dy;
    col += dx;
    if (row < 0) row = 0; if (row >= MAX_ROWS) row = MAX_ROWS - 1;
    if (col < 0) col = 0; if (col >= MAX_COLS) col = MAX_COLS - 1;
    set_cursor_offset(get_offset(col, row));
}

int screen_get_cursor_offset(void) { return get_cursor_offset(); }
void screen_set_cursor_offset(int offset) { set_cursor_offset(offset); }